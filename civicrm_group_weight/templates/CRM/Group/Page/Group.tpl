...
{php}    
  $var = $this->get_template_vars('rows');             
  foreach($var as $k => $v) {
      $sql =  "SELECT * FROM `mtl_civicrm_group_weight` w LEFT JOIN civicrm_group g ON w.entity_id =g.id WHERE w.entity_id=".$k;
      $dao = CRM_Core_DAO::executeQuery($sql);  
      if($dao->fetch()) {
          if($dao->weight) {             
              $var[$k]['weight'] = $dao->weight;
              $var[$k]['weight'] .="<br> Parents:".$dao->parents;
          }    
      }    
   }             
   $this->assign('rows',$var );
{/php} 
